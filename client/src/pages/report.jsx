...
const getTrend = (current, previous) => {
  ...
  if (diff > 0) return { type: 'up', text: `${diff} more than last week`, icon: TrendingUp, color: 'text-emerald-500' };
  if (diff < 0) return { type: 'down', text: `${Math.abs(diff)} fewer than last week`, icon: TrendingDown, color: 'text-rose-500' };
  return { type: 'neutral', text: 'Same as last week', icon: Minus, color: 'text-slate-400' };
};
...
const StatCard = ({ title, icon: Icon, mainValue, subValue, trend }) => (
  <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col justify-between h-52 md:h-auto md:flex-1">
    ...
    <div className="mt-4 md:mt-6">
      <div className="text-6xl md:text-7xl font-bold tracking-tighter text-slate-900">{mainValue}</div>
      {trend ? (
        ...
        <div className="text-sm text-slate-400 mt-2 font-medium">{subValue}</div>
      )}
    </div>
  </div>
);
...
const Report = () => {
  ...
  if (loading) {
    contentArea = <div className="flex h-64 items-center justify-center text-slate-500 font-medium animate-pulse">Loading analysis...</div>;
  } else if (error) {
    contentArea = (
      <div className="flex flex-col h-64 items-center justify-center text-red-500 font-medium gap-4">
        <p>{error}</p>
        <button onClick={fetchTasks} className="px-4 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors">
          Retry
        </button>
      </div>
    );
  } else if (!stats) {
    contentArea = <div className="flex h-64 items-center justify-center text-slate-500">Add some tasks to see your analytics.</div>;
  } else {
    contentArea = (
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm min-h-[300px]">
            <div className="flex items-center justify-between mb-8">
              <h2 className="font-bold text-lg flex items-center gap-2">
                <BarChart2 className="w-5 h-5 text-indigo-500" /> Activity Overview
              </h2>
              {stats.hasActivity && (
                  ...
                  <div key={i} className="flex flex-col items-center flex-1 group h-full">
                    <div className="relative w-full flex justify-center h-full items-end">
                      <div 
                        ...
                        style={{ height: `${(day.count / stats.maxDailyTasks) * 100}%`, minHeight: day.count > 0 ? '12%' : '0%' }}
                      ></div>
                    </div>
      ...
      <div className="max-w-6xl mx-auto mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center bg-white rounded-full shadow-sm border border-slate-200 p-1">
          ...
          <span className="px-4 font-medium text-sm text-slate-700 min-w-[150px] text-center">
            {weekOffset === 0 ? "Current Week" : `${formatDate(displayWindow.start)} - ${formatDate(displayWindow.end)}`}
          </span>
