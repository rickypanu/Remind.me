...
async def check_telegram_status(user_id: str):
    """Checks if the user has connected Telegram and returns their details."""
...
async def get_telegram_link(payload: TelegramPayload):
    """Generates deep-link URL for opening the bot in Telegram."""
    if not BOT_USERNAME:
...
async def telegram_webhook(request: Request):
    """Handles Telegram Webhook updates for /start and phone contact sharing."""
    ...
    if "message" in data:
        ...
        # 1. User taps /start USER_ID in Telegram
        if text.startswith("/start"):
            parts = text.split(" ")
            if len(parts) > 1:
                user_id = parts[1]

                # Link chat_id to the user record
                ...
                # Prompt user to optionally share phone number
                payload = {
                    "chat_id": chat_id,
        ...
        # 2. User taps 'Share Phone Number'
...
async def disconnect_telegram(payload: TelegramPayload):
    """Unlinks Telegram chat ID and phone number from user account."""
    await db["users"].update_one(
        {"_id": payload.userId},
